...
const ShiftSwapRequests = () => {
  // Pagination state for shiftswaprequests
  const [currentPage_shiftswaprequests, setCurrentPage_shiftswaprequests] = useState(1);
  const [rowsPerPage_shiftswaprequests, setRowsPerPage_shiftswaprequests] = useState(10);
  const [searchQuery_shiftswaprequests, setSearchQuery_shiftswaprequests] = useState('');
  return (
    <>
			...
			<div className="content">
				...
				<PageHeader 
					...
					breadcrumbs={[
						{ label: 'Dashboard' },
						{ label: 'Attendance' },
						{ label: 'Shift Swap Requests', active: true }
					]}
				>
								...
								<div className="d-flex align-items-center justify-content-between flex-wrap row-gap-2 mb-3 px-3 pt-3">
									<div className="d-flex align-items-center">
										...
										<select
											...
											value={rowsPerPage_shiftswaprequests}
											onChange={(e) => { setRowsPerPage_shiftswaprequests(Number(e.target.value)); setCurrentPage_shiftswaprequests(1); }}
										>
									...
									<div className="input-icon-start position-relative">
										...
										<input
											...
											placeholder="Search"
											value={searchQuery_shiftswaprequests}
											onChange={(e) => { setSearchQuery_shiftswaprequests(e.target.value); setCurrentPage_shiftswaprequests(1); }}
										/>
									</div>
								</div>
							...
							<table className="table datatable">
								...
								<div className="d-flex align-items-center justify-content-between flex-wrap row-gap-2 px-3 py-3 border-top">
									<p className="mb-0 text-gray-9 fs-14">
										Showing {Math.min((currentPage_shiftswaprequests - 1) * rowsPerPage_shiftswaprequests + 1, 11)}-{Math.min(currentPage_shiftswaprequests * rowsPerPage_shiftswaprequests, 11)} of 11 entries
									</p>
									<ul className="pagination mb-0">
										<li className={`page-item ${currentPage_shiftswaprequests === 1 ? 'disabled' : ''}`}>
											<button className="page-link" onClick={() => setCurrentPage_shiftswaprequests(p => Math.max(p - 1, 1))}>
												<i className="ti ti-chevron-left"></i>
											</button>
										</li>
										{[1, 2].map(page => (
											<li key={page} className={`page-item ${currentPage_shiftswaprequests === page ? 'active' : ''}`}>
												<button className="page-link" onClick={() => setCurrentPage_shiftswaprequests(page)}>{page}</button>
											</li>
										))}
										<li className={`page-item ${currentPage_shiftswaprequests === 2 ? 'disabled' : ''}`}>
											<button className="page-link" onClick={() => setCurrentPage_shiftswaprequests(p => Math.min(p + 1, 2))}>
