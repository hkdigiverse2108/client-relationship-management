...
const Trainers = () => {
  // Pagination state for trainers
  const [currentPage_trainers, setCurrentPage_trainers] = useState(1);
  const [rowsPerPage_trainers, setRowsPerPage_trainers] = useState(10);
  const [searchQuery_trainers, setSearchQuery_trainers] = useState('');
  return (
    <>
					...
					<div className="mb-2">
							<a href="#" data-bs-toggle="modal" data-bs-target="#add_trainer"
								className="btn btn-primary d-flex align-items-center"><i
									className="ti ti-circle-plus me-2"></i>Add Trainer</a>
						</div>
								...
								<div className="d-flex align-items-center justify-content-between flex-wrap row-gap-2 mb-3 px-3 pt-3">
									<div className="d-flex align-items-center">
										...
										<select
											...
											value={rowsPerPage_trainers}
											onChange={(e) => { setRowsPerPage_trainers(Number(e.target.value)); setCurrentPage_trainers(1); }}
										>
									...
									<div className="input-icon-start position-relative">
										...
										<input
											...
											placeholder="Search"
											value={searchQuery_trainers}
											onChange={(e) => { setSearchQuery_trainers(e.target.value); setCurrentPage_trainers(1); }}
										/>
									</div>
								</div>
							...
							<table className="table datatable">
										...
										<td>
											<div className="action-icon d-inline-flex">
												<a href="#" className="me-2" data-bs-toggle="modal"
													data-bs-target="#edit_trainer"><i className="ti ti-edit"></i></a>
								...
								<div className="d-flex align-items-center justify-content-between flex-wrap row-gap-2 px-3 py-3 border-top">
									<p className="mb-0 text-gray-9 fs-14">
										Showing {Math.min((currentPage_trainers - 1) * rowsPerPage_trainers + 1, 11)}-{Math.min(currentPage_trainers * rowsPerPage_trainers, 11)} of 11 entries
									</p>
									<ul className="pagination mb-0">
										<li className={`page-item ${currentPage_trainers === 1 ? 'disabled' : ''}`}>
											<button className="page-link" onClick={() => setCurrentPage_trainers(p => Math.max(p - 1, 1))}>
												<i className="ti ti-chevron-left"></i>
											</button>
										</li>
										{[1, 2].map(page => (
											<li key={page} className={`page-item ${currentPage_trainers === page ? 'active' : ''}`}>
												<button className="page-link" onClick={() => setCurrentPage_trainers(page)}>{page}</button>
											</li>
										))}
										<li className={`page-item ${currentPage_trainers === 2 ? 'disabled' : ''}`}>
											<button className="page-link" onClick={() => setCurrentPage_trainers(p => Math.min(p + 1, 2))}>
