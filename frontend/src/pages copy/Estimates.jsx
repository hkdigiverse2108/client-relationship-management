...
const Estimates = () => {
  // Pagination state for estimates
  const [currentPage_estimates, setCurrentPage_estimates] = useState(1);
  const [rowsPerPage_estimates, setRowsPerPage_estimates] = useState(10);
  const [searchQuery_estimates, setSearchQuery_estimates] = useState('');
  return (
    <>
				...
				>
					<div className="mb-2">
							<a href="#" className="btn btn-primary d-flex align-items-center" data-bs-toggle="modal"
								data-bs-target="#add_estimate"><i className="ti ti-circle-plus me-2"></i>Add Estimates</a>
						</div>
								...
								<div className="d-flex align-items-center justify-content-between flex-wrap row-gap-2 mb-3 px-3 pt-3">
									<div className="d-flex align-items-center">
										...
										<select
											...
											value={rowsPerPage_estimates}
											onChange={(e) => { setRowsPerPage_estimates(Number(e.target.value)); setCurrentPage_estimates(1); }}
										>
									...
									<div className="input-icon-start position-relative">
										...
										<input
											...
											placeholder="Search"
											value={searchQuery_estimates}
											onChange={(e) => { setSearchQuery_estimates(e.target.value); setCurrentPage_estimates(1); }}
										/>
									</div>
								</div>
							...
							<table className="table datatable">
										...
										<td>
											<div className="action-icon d-inline-flex">
												<a href="#" className="me-2" data-bs-toggle="modal"
													data-bs-target="#edit_estimate"><i className="ti ti-edit"></i></a>
								...
								<div className="d-flex align-items-center justify-content-between flex-wrap row-gap-2 px-3 py-3 border-top">
									<p className="mb-0 text-gray-9 fs-14">
										Showing {Math.min((currentPage_estimates - 1) * rowsPerPage_estimates + 1, 11)}-{Math.min(currentPage_estimates * rowsPerPage_estimates, 11)} of 11 entries
									</p>
									<ul className="pagination mb-0">
										<li className={`page-item ${currentPage_estimates === 1 ? 'disabled' : ''}`}>
											<button className="page-link" onClick={() => setCurrentPage_estimates(p => Math.max(p - 1, 1))}>
												<i className="ti ti-chevron-left"></i>
											</button>
										</li>
										{[1, 2].map(page => (
											<li key={page} className={`page-item ${currentPage_estimates === page ? 'active' : ''}`}>
												<button className="page-link" onClick={() => setCurrentPage_estimates(page)}>{page}</button>
											</li>
										))}
										<li className={`page-item ${currentPage_estimates === 2 ? 'disabled' : ''}`}>
											<button className="page-link" onClick={() => setCurrentPage_estimates(p => Math.min(p + 1, 2))}>
