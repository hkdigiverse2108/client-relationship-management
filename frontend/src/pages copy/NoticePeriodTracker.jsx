...
const NoticePeriodTracker = () => {
  // Pagination state for noticeperiodtracker
  const [currentPage_noticeperiodtracker, setCurrentPage_noticeperiodtracker] = useState(1);
  const [rowsPerPage_noticeperiodtracker, setRowsPerPage_noticeperiodtracker] = useState(10);
  const [searchQuery_noticeperiodtracker, setSearchQuery_noticeperiodtracker] = useState('');
  return (
    <>
			...
			<div className="content">
				...
				<PageHeader 
					...
					breadcrumbs={[
						{ label: 'Dashboard' },
						{ label: 'HRM' },
						{ label: 'Notice Period Tracker', active: true }
					]}
				>
								...
								<div className="d-flex align-items-center justify-content-between flex-wrap row-gap-2 mb-3 px-3 pt-3">
									<div className="d-flex align-items-center">
										...
										<select
											...
											value={rowsPerPage_noticeperiodtracker}
											onChange={(e) => { setRowsPerPage_noticeperiodtracker(Number(e.target.value)); setCurrentPage_noticeperiodtracker(1); }}
										>
									...
									<div className="input-icon-start position-relative">
										...
										<input
											...
											placeholder="Search"
											value={searchQuery_noticeperiodtracker}
											onChange={(e) => { setSearchQuery_noticeperiodtracker(e.target.value); setCurrentPage_noticeperiodtracker(1); }}
										/>
									</div>
								</div>
							...
							<table className="table datatable">
								...
								<tbody>
									...
									<tr>
										...
										<td>
											<span
												className="badge badge-purple-transparent d-inline-flex align-items-center badge-xs">
												Closing Soon
											</span>
										</td>
										<td>
								...
								<div className="d-flex align-items-center justify-content-between flex-wrap row-gap-2 px-3 py-3 border-top">
									<p className="mb-0 text-gray-9 fs-14">
										Showing {Math.min((currentPage_noticeperiodtracker - 1) * rowsPerPage_noticeperiodtracker + 1, 11)}-{Math.min(currentPage_noticeperiodtracker * rowsPerPage_noticeperiodtracker, 11)} of 11 entries
									</p>
									<ul className="pagination mb-0">
										<li className={`page-item ${currentPage_noticeperiodtracker === 1 ? 'disabled' : ''}`}>
											<button className="page-link" onClick={() => setCurrentPage_noticeperiodtracker(p => Math.max(p - 1, 1))}>
												<i className="ti ti-chevron-left"></i>
											</button>
										</li>
										{[1, 2].map(page => (
											<li key={page} className={`page-item ${currentPage_noticeperiodtracker === page ? 'active' : ''}`}>
												<button className="page-link" onClick={() => setCurrentPage_noticeperiodtracker(page)}>{page}</button>
											</li>
										))}
										<li className={`page-item ${currentPage_noticeperiodtracker === 2 ? 'disabled' : ''}`}>
											<button className="page-link" onClick={() => setCurrentPage_noticeperiodtracker(p => Math.min(p + 1, 2))}>
