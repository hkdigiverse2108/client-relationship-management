...
def get_email_template(title: str, content_html: str, content_text: str):
    ...
    <body>
        <div class="container">
            ...
            <div class="footer">
                <p>This is an automated message from HK DigiVerse CRM. Please do not reply directly to this email.</p>
                <p>&copy; {os.getenv('YEAR', '2026')} HK DigiVerse CRM Systems. All rights reserved.</p>
            </div>
        </div>
    </body>
    </html>
...
--------------------------------------------------
This is an automated message from HK DigiVerse CRM. Please do not reply directly to this email.
(c) HK DigiVerse CRM Systems. All rights reserved.
    """
    return html, text
...
def send_email(to_email: str, subject: str, title: str, content_html: str, content_text: str):
    ...
    try:
        ...
        msg['From'] = f"HK DigiVerse CRM <{EMAIL_FROM}>"
        msg['To'] = to_email
        msg['Subject'] = subject
...
def send_new_account_email(to_email: str, name: str, password: str, login_url: str):
    subject = "Welcome to HK DigiVerse CRM - Your Account Setup is Complete"
...
def send_otp_email(to_email: str, otp: str):
    subject = "HK DigiVerse CRM - Password Reset Verification Code"
...
This code is valid for 2 minutes. If you did not request a password reset, please ignore this email.
    """
    
    return send_email(to_email, subject, title, content_html, content_text)

def send_password_changed_by_admin_email(to_email: str, name: str, new_password: str, login_url: str):
    subject = "HK DigiVerse CRM - Security Notice: Your Password Has Been Updated"
    title = f"Hello {name},"
...
def send_unregistered_login_attempt_email(to_email: str):
    admin_email = os.getenv("EMAIL_FROM", "admin@aiocrm.com")
    subject = "HK DigiVerse CRM - Login Attempt with Unregistered Email"
    title = "Login Attempt Notice"
    
    content_html = f"""
        <p>We noticed someone attempted to reset the password or log in to HK DigiVerse CRM using this email address.</p>
        <p>However, this email address is <strong>not registered</strong> in our system.</p>
        <div style="background: #f8f9fa; padding: 15px; border-radius: 5px; margin: 20px 0; border-left: 4px solid #ffc107;">
            <p style="margin: 0;">If you wish to use our CRM system, please contact your administrator at <a href="mailto:{admin_email}" style="color: #0d6efd; text-decoration: none;">{admin_email}</a> to request an account.</p>
        </div>
        <p>If you didn't attempt this login, you can safely ignore this email.</p>
    """
    
    content_text = f"""
We noticed someone attempted to reset the password or log in to HK DigiVerse CRM using this email address.
However, this email address is not registered in our system.

If you wish to use our CRM system, please contact your administrator at {admin_email} to request an account.

If you didn't attempt this login, you can safely ignore this email.
    """
